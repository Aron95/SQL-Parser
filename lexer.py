...
teststring ='SELECT DISTINCT column1, column2 FROM   table_name;SELECT row2 FROM table;Patrick stinkt;'

KEYWORD = ["SELECT","FROM","DISTINCT","CREATE","CREATE DATABASE","INSERT","ALTER","ADD","UPDATE","DELETE","AND","OR"]
...
for i,char in enumerate(teststring):
    ...
    # if char is not a white space adds it to lexeme
    if char != white_space:
        lexeme += char
